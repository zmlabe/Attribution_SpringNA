...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/ObsRegressions/' 
directorydata = '/work/Zachary.Labe/Data/' 
directorydata2 = '/work/Zachary.Labe/Data/ClimateIndices/'
directorydataoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/RegressionPatternsObs/'

### Loop through indices
indicesdir = ['AL','SND_BoxNA','PNA','NAO','NPI','PCHT2M','PCHZ30','PCHZ100','PCHZ1000','SHI','UBI']
indices = ['AL','SND_BoxNA','PNAmodified','NAOmodified','NPI','PCHT2M','PCHZ30','PCHZ100','PCHZ1000','SHI','UBI']
indicesvari = ['SLP','SND','Z500','SLP','SLP','T2M','Z30','Z100','Z1000','SLP','Z500']
...
for mo in range(len(monthsall)):
    ...
    for iii in range(len(indices)):
        ...
        NAOd = np.genfromtxt(directorydata2 + '%s/%s_%s_%s_%s-%s_detrended.txt' % (indicesdir[iii],indices[iii],indicesvari[iii],monq,yearmin,yearmax),unpack=True)
        years_NAO = NAOd[1,:]
        yearclimoq = np.where((years>=1981) & (years<=2010))[0]
        years_NAOq = years_NAO[yearclimoq]
        meanclimo = np.nanmean(years_NAOq)
        stdclimo = np.nanstd(years_NAOq)
        NAO = (years_NAO - meanclimo)/stdclimo
        
        ### Calculate regression
        slope,intercept,rvalue,pvalue,stderr = regressData(NAO,vardt,'obs')
        ...
        plt.title(r'\textbf{%s-%s of %s(%s)-T2M(%s)}' % (yearmin,yearmax,indices[iii],monq,sliceperiod),fontsize=15,color='k')
        ...
        plt.savefig(directoryfigure + 'RegressionObs_pattern_%s-%s_%s-%s.png' % (variqM,sliceperiod,indices[iii],monq),dpi=300)
        
        ### Save file
        np.savez(directorydataoutput + 'RegressionObs_pattern_%s-%s_%s-%s.npz' % (variqM,sliceperiod,indices[iii],monq),
