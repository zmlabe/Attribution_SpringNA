...
for mo in range(len(monthq)):                                                                                                                         
    ...
    m = Basemap(projection='ortho',lon_0=265,lat_0=30,resolution='l',area_thresh=10000)
    ...
    ax.annotate(r'\textbf{%s}' % monthq[mo],xy=(0,0),xytext=(-0.05,0.84),
              textcoords='axes fraction',color='k',fontsize=9,
              rotation=0,ha='center',va='center')
    ax.annotate(r'\textbf{[%s]}' % letters[mo],xy=(0,0),xytext=(1.01,0.84),
              textcoords='axes fraction',color='dimgrey',fontsize=9,
              rotation=0,ha='center',va='center')
    
cbar_ax1 = fig.add_axes([0.398,0.06,0.2,0.025])                
...
plt.savefig(directoryfigure + 'BaselineMapDifferences_Monthly_NH.png',dpi=300)
