...
### Read in data
trend_obs_all = np.loadtxt(directoryoutput + 'SlopesTrends/Slopes_%s_obs_%s-%s.txt' % (slicemonth,1979,2014))
...
model_obs_rfy = ['CAM4(%s)'% (year[1][0].max()),
                 ...
                 'CAM5(%s)'% (year[1][2].max()),
                 'SPEAR(%s)'% (year[3][0].max())]
model_clim_polary = ['CAM4(%s)'% (year[2][0].max()),
                     'ECHAM5(%s)'% (year[2][1].max()),
                     'CAM5(%s)'% (year[2][2].max()),
                     'GFS(%s)'% (year[2][3].max())]
modely = [['CAM4','ECHAM5','ESRL-CAM5'],['CAM4','ECHAM5','ESRL-CAM5','SPEAR'],['CAM4','ECHAM5','ESRL-CAM5','ESRL-GFS']]
modeldata = [trend_all[0],[trend_all[1][0],trend_all[1][1],trend_all[1][2],trend_all[3][0]],trend_all[2]]
...
plt.tight_layout()
plt.savefig(directoryfigure + 'AMIPS_TrendViolins_%s_1979-2014.png' % (slicemonth),dpi=300)
