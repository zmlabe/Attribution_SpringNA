"""
Calculate regression fields for AMIP experiments using 1880s rf
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/AMIPS/RegressionPatternsAMIPS/' 
...
for i in range(len(indices)):
    for mm in range(len(models)):
        ...
        amip = np.load(directoryoutput + 'ClimateIndices_AMIPSPEAR-1880rf_%s_1979-2019.npz' % mon_index)
        ...
        plt.title(r'\textbf{Regression Pattern for %s(%s) on %s(%s) using %s(1880)}' % (variq,mon_t2m,indices[i],mon_index,models[mm]),fontsize=8,color='k')
        ...
        plt.savefig(directoryfigure + 'RegressionAMIP-1880rf_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),dpi=300)
        
        ### Save file
        np.savez(directorydataoutput + 'RegressionAMIP-1880rf_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),
