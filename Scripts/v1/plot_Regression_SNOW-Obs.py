"""
Calculate regression maps of SND_BoxNA onto T2M maps
...
for mo in range(len(monthsall)):
    ...
    SND_BoxNAd = np.genfromtxt(directorydata2 + 'SND_BoxNA/SND_BoxNA_%s_%s_%s-%s_detrended.txt' % ('SND',monq,yearmin,yearmax),unpack=True)
    years_SND_BoxNA = SND_BoxNAd[0,:]
    SND_BoxNA = sts.zscore(SND_BoxNAd[1,:])
    
    ### Calculate regression
    slope,intercept,rvalue,pvalue,stderr = regressData(SND_BoxNA,vardt,'obs')
    ...
    plt.title(r'\textbf{%s-%s of SND_BoxNAi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
    ...
    plt.savefig(directoryfigure + 'RegressionMap_%s-%s_SND_BoxNAi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
