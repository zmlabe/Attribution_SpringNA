"""
Calculate tropical teleconnections for the AMIP experiments
...
for mn in range(len(monthsall)):
    monq = monthsall[mn]
    scenario = ['amip_1880s_rf','spear_obs_rf']
    ...
    def calcTeleconnections(scenario,model,indicesvari,indices,monq,years):
        ...
        ###############################################################################
            ...
            ### Calculate indices for each ensemble member of ECHAM5
            WPPRECT_echam = np.empty((len(echam_dt),years.shape[0]))
            for ens in range(len(echam_dt)):
                WPPRECT_echam[ens,:] = calc_WPPRECT(echam_dt[ens,:,:,:],echam_lat,echam_lon,years)
                
            ### Calculate indices for each ensemble member of CAM5
            WPPRECT_cam = np.empty((len(cam_dt),years.shape[0]))
            for ens in range(len(cam_dt)):
                WPPRECT_cam[ens,:] = calc_WPPRECT(cam_dt[ens,:,:,:],cam_lat,cam_lon,years)
            ...
            ### Return common index
            indexz_echam = WPPRECT_echam
            indexz_cam = WPPRECT_cam
        ...
        ###############################################################################
            ...
            ### Calculate indices for each ensemble member of ECHAM5
            NECPSST_echam = np.empty((len(echam_dt),years.shape[0]))
            for ens in range(len(echam_dt)):
                NECPSST_echam[ens,:] = calc_NECPSST(echam_dt[ens,:,:,:],echam_lat,echam_lon,years)
                
            ### Calculate indices for each ensemble member of CAM5
            NECPSST_cam = np.empty((len(cam_dt),years.shape[0]))
            for ens in range(len(cam_dt)):
                NECPSST_cam[ens,:] = calc_NECPSST(cam_dt[ens,:,:,:],cam_lat,cam_lon,years)
            ...
            ### Return common index
            indexz_echam = NECPSST_echam
            indexz_cam = NECPSST_cam
    ...
    if scenario[0] == 'amip_1880s_rf':
        np.savez(directoryoutput + 'ClimateIndices-Tropical_AMIPSPEAR-1880rf_%s_1979-2019.npz' % monq,
                 echam=data_echam,cam=data_cam,spear=data_spear,
                 indices=indices,indicesvari=indicesvari,years=years)
    else:
        np.savez(directoryoutput + 'ClimateIndices-Tropical_AMIPSPEAR_%s_1979-2019.npz' % monq,
                 echam=data_echam,cam=data_cam,spear=data_spear,
                 indices=indices,indicesvari=indicesvari,years=years)
