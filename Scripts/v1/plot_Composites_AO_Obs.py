"""
Plot composites of different AO combinations during period of interest
...
for v in range(len(varplot)):
    ...
    posAOq = np.where((AO > 0.25))[0]
    negAOq = np.where((AO < -0.25))[0]
    
    ### Calculate mean composites
    posAO = np.nanmean(vardt[posAOq,:,:],axis=0)
    negAO = np.nanmean(vardt[negAOq,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posAOq),len(negAOq)]
    allcomposites = [posAO,negAO]
    comptitles = ['+AO','-AO']
    
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ### Plot composite maps of different AO/AO combinations
    if variqM == 'T2M':
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_AO-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_AO-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
