"""
Plot composites of different NAO combinations during period of interest
...
for v in range(len(varplot)):
    ...
    posNAOq = np.where((NAO > 0.25))[0]
    negNAOq = np.where((NAO < -0.25))[0]
    
    ### Calculate mean composites
    posNAO = np.nanmean(vardt[posNAOq,:,:],axis=0)
    negNAO = np.nanmean(vardt[negNAOq,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posNAOq),len(negNAOq)]
    allcomposites = [posNAO,negNAO]
    comptitles = ['+NAO','-NAO']
    
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ### Plot composite maps of different AO/NAO combinations
    if variqM == 'T2M':
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_NAO-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_NAO-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
