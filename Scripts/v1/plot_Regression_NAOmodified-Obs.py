"""
Calculate regression maps of NAO onto T2M maps
...
NAOd = np.genfromtxt(directorydata2 + 'NAO/NAOmodified_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,yearmin,yearmax),unpack=True)
years_NAO = NAOd[0,:]
NAO = sts.zscore(NAOd[1,:])
...
plt.title(r'\textbf{%s-%s of NAOi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap-ModifiedIndexNAO_%s-%s_NAOi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
