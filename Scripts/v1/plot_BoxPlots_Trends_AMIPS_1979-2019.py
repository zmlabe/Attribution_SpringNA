...
### Read in data
trend_obs_all = np.loadtxt('/home/Zachary.Labe/Research/Attribution_SpringNA/Data/SlopesTrends/Slopes_%s_obs_%s-%s.txt' % (slicemonth,1979,2019))
amipdata = np.load(directoryoutput + 'AMIP_AMIPSPEAR_Trends_%s_%s.npz' % (slicemonth,variq),
                   allow_pickle=True)
...
model_speary = ['SPEAR']
modely = [['ECHAM5','ESRL-CAM5'],['ECHAM5','ESRL-CAM5','SPEAR']]
modeldata = [trend_all[0],[trend_all[1][0],trend_all[1][1],trend_all[2][0]]]
...
for vv in range(len(modeldata)):
    if vv < 1:
        ...
        datum = [decade*10 for decade in modeldata[vv]]
...
plt.tight_layout()
plt.savefig(directoryfigure + 'AMIPS-AMIPSPEAR_TrendViolins_%s_all.png' % (slicemonth),dpi=300)
...
np.savez(directoryoutput + 'EnsembleMembers_SimilarTrends_AMIPSPEAR_%s_%s_all.npz' % (variq,slicemonth),
