...
import read_SPEAR_LOW_Hindcast_65lev as SP
...
### Read in obs
latobs,lonobs,levobs,obs = ER.read_ERA5_monthlyHindcast(variq,'/work/Zachary.Labe/Data/',monthhind,slicenan,'surface')
...
lat,lon,spear = SP.read_SPEAR_LOW_Hindcast65lev(directorydata,variq,monthhind,slicenan,15)
...
trendhind = UT.linearTrendR(spear_hind,years,'surface',yearmin,yearmax) * 10.

# trendhindall = np.empty((15,lat.shape[0],lon.shape[0]))
# for e in range(15):
#     trendhindall[e,:,:] = UT.linearTrendR(spear_time[:,e,:,:],years,'surface',yearmin,yearmax)
# trendhind = np.nanmean(trendhindall,axis=0)*10.
...
plt.savefig(directoryfigure + 'TrendsMap_%s_Hindcast-65lev_init-%s_%s_%s-%s.png'% (slicemonth,monthhind,variq,yearmin,yearmax),dpi=300)
