"""
Plot monthly trends in the Arctic for SLP
...
amip = np.load(directorydataAMIP + 'AMIP_SPEARval_MonthlyData_AllMonths_SLP.npz')
...
limit = np.arange(-2.5,2.6,0.2)
barlim = np.round(np.arange(-2.5,2.6,0.5),2)

fig = plt.figure(figsize=(8,2.5))
label = r'\textbf{SLP Trend [hPa/decade] for %s--%s}' % (yearmin,yearmax)
...
for i in range(len(monthq)*2):
    ...
    m = Basemap(projection='npstere',boundinglat=47,lon_0=270,resolution='l',
...
label = r'\textbf{SLP Trend Difference [hPa/decade] for %s--%s}' % (yearmin,yearmax)
