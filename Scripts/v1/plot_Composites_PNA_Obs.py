"""
Plot composites of different PNA combinations during period of interest
...
for v in range(len(varplot)):
    ...
    posPNAq = np.where((PNA > 0.25))[0]
    negPNAq = np.where((PNA < -0.25))[0]
    
    ### Calculate mean composites
    posPNA = np.nanmean(vardt[posPNAq,:,:],axis=0)
    negPNA = np.nanmean(vardt[negPNAq,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posPNAq),len(negPNAq)]
    allcomposites = [posPNA,negPNA]
    comptitles = ['+PNA','-PNA']
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_PNA-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_PNA-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
