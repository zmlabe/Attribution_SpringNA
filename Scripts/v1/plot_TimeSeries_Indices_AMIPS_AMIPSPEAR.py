...
for i in range(len(indices)):
    ...
    for m in range(len(models)):
        ...
        if i < 1:
            ...
            amip = np.load(directoryoutput + 'ClimateIndices-T2M_BoxNA_AMIPSPEAR_%s_1979-2019.npz' % monq)
            amipi = amip['%s' % modelname[m]][i,:]            
        else:
            ### Read in AMIP data
            directoryoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/Teleconnections/'
            amip = np.load(directoryoutput + 'ClimateIndices_AMIPSPEAR_%s_1979-2019.npz' % monq)
