"""
Plot regression patterns for SPEAR-MED over 1979 to 2019 for T2M_BoxNA
...
directoryoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/RegressionPatternsCoupledSPEAR/T2M_BoxNA/'
    ...
    for moo in range(len(monthsall)):
        ...
        for iii in range(len(variregress)):
            ...
            for mm in range(len(models)):
                ...
                amip = np.load(directorydata + 'ClimateIndices_coupledSPEAR-T2M_BoxNA_%s_1979-2019.npz' % mon_index)
                ...
                plt.savefig(directoryfigure + 'RegressionCoupledSPEAR_T2M_BoxNApattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),dpi=300)
                
                ### Save file
                np.savez(directoryoutput + 'RegressionCoupledSPEAR_T2M_BoxNApattern_%s-%s_%s-%s_%s.npz' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),
