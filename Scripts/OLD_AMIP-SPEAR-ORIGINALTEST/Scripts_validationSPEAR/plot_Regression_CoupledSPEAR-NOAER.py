...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/CoupledSPEAR/RegressionPlots_Extratropics-NOAER/' 
...
indices = ['AL','PNAmodified','NAOmodified','NPI','PCHT2M','PCHZ30','PCHZ100','PCHZ1000','SHI','UBI']
...
for i in range(len(indices)):
    for mm in range(len(models)):
        ...
        amip = np.load(directorydata + 'ClimateIndices_coupledSPEAR-NOAER_%s_1979-2019.npz' % mon_index)
        amipi = amip['spear'][i,:,:]
        ...
        plt.savefig(directoryfigure + 'RegressionCoupledSPEAR-NOAER_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),dpi=300)
        
        ### Save file
        np.savez(directoryoutput + 'RegressionCoupledSPEAR-NOAER_pattern_%s-%s_%s-%s_%s.npz' % (variq,mon_t2m,indices[i],mon_index,models[mm]),
