    ...
    for moo in range(len(monthsall)):
        ...
        for iii in range(len(variregress)):
            ...
            for mm in range(len(models)):
                ...
                amip = np.load(directoryoutput + 'ClimateIndices-T2M_BoxNA_ValidationSPEAR_%s_1979-2019.npz' % mon_index)
                ...
                plt.savefig(directoryfigure + 'RegressionAMIP_T2M_BoxNApattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),dpi=300)
                
                ### Save file
                np.savez(directorydataoutput + 'RegressionAMIP_T2M_BoxNApattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),
