...
for mn in range(len(monthsall)):
    ...
    if scenario[0] == 'amip_1880s_rf':
        np.savez(directoryoutput + 'ClimateIndices-T2M_BoxNA_ValidationSPEAR-1880rf_%s_1979-2019.npz' % monq,
                 echam=data_echam,cam=data_cam,spear=data_spear,
                 indices=indices,indicesvari=indicesvari,years=years)
    else:
        np.savez(directoryoutput + 'ClimateIndices-T2M_BoxNA_ValidationSPEAR_%s_1979-2019.npz' % monq,
                 echam=data_echam,cam=data_cam,spear=data_spear,
                 indices=indices,indicesvari=indicesvari,years=years)
