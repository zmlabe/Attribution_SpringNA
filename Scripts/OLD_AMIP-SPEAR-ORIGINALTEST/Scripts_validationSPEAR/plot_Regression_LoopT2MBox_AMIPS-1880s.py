"""
Plot regression patterns for amips using the T2M_BoxNA (1880s)
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/validationSPEAR/RegressionPatternsAMIPS/T2M_BoxNA/' 
    ...
    for moo in range(len(monthsall)):
        ...
        for iii in range(len(variregress)):
            ...
            for mm in range(len(models)):
                ...
                amip = np.load(directoryoutput + 'ClimateIndices-T2M_BoxNA_ValidationSPEAR-1880rf_%s_1979-2019.npz' % mon_index)
                ...
                plt.title(r'\textbf{Regression Pattern for %s(%s) on %s(%s) using %s(1880)}' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),fontsize=8,color='k')
                ...
                plt.savefig(directoryfigure + 'RegressionAMIP-1880rf_T2M_BoxNApattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),dpi=300)
                
                ### Save file
                np.savez(directorydataoutput + 'RegressionAMIP-1880rf_T2M_BoxNApattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,'T2M_BoxNA',mon_index,models[mm]),
