...
### February to April mean for 2022
diffmean = np.nanmean(anoms[-1,1:4,:,:],axis=0)

###############################################################################
###############################################################################
###############################################################################
### Plot subplot of different SAI analysis
limit = np.arange(-5,5.01,0.02)
barlim = np.round(np.arange(-5,6,1),2)

fig = plt.figure(figsize=(6,6))
label = r'\textbf{Anomalies [$^{\circ}$]}'
...
plt.title(r'\textbf{FEBRUARY -- APRIL 2022}',fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'AnomaliesMap_FMA2022.png',dpi=300)
