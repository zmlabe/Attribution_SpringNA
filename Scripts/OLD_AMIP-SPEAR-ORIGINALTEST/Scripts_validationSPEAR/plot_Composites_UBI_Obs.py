"""
Plot composites of different UBI combinations during period of interest
...
for v in range(len(varplot)):
    ...
    UBId = np.genfromtxt(directorydata2 + 'UBI/UBI_%s_%s_%s-%s_detrended.txt' % ('Z500',monq,
                                                                    yearmin,yearmax),
                                                                    unpack=True)
    years_UBI = UBId[0,:]
    UBI = sts.zscore(UBId[1,:])
    
    posUBIq = np.where((UBI > 0.25))[0]
    negUBIq = np.where((UBI < -0.25))[0]

    ### Calculate mean composites
    posUBI = np.nanmean(vardt[posUBIq,:,:],axis=0)
    negUBI = np.nanmean(vardt[negUBIq,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posUBIq),len(negUBIq)]
    allcomposites = [posUBI,negUBI]
    comptitles = ['+UBI','-UBI']
    
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ### Plot composite maps of different UBI/UBI combinations
    if variqM == 'T2M':
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_UBI-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_UBI-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
