"""
Plot monthly trends in the Arctic for the PCHTHICK
...
PCHTHICKd = np.genfromtxt(directorydata + 'PCHTHICK/PCHTHICK_%s_%s_%s-%s.txt' % ('THICK',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_PCHTHICK = PCHTHICKd[0,:]
PCHTHICK = sts.zscore(PCHTHICKd[1,:])
    
### Calculate trends
slope_PCHTHICK,intercept_PCHTHICK,r_PCHTHICK,p_PCHTHICK,se_PCHTHICK = sts.linregress(years,PCHTHICK)

### Calculate correlations
corr_PCHTHICK,pcorr_PCHTHICK = sts.pearsonr(obsz,PCHTHICK)
...
rects = plt.bar(years,PCHTHICK,align='center',zorder=2,color='crimson',label=r'PCH-THICK',clip_on=False)
plt.plot(years,obsz,linestyle='--',dashes=(1,0.3),color='k',linewidth=2,clip_on=False,label='ERA5-THICK')
...
plt.savefig(directoryfigure + 'PCHTHICK_Indices-Obs_%s' % monq,dpi=300)
