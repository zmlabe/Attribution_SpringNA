"""
Calculate regression maps of WPPRECT onto T2M maps
...
WPPRECTd = np.genfromtxt(directorydata2 + 'WPPRECT/WPPRECT_%s_%s_%s-%s_detrended.txt' % ('P',monq,yearmin,yearmax),unpack=True)
years_WPPRECT = WPPRECTd[0,:]
WPPRECT = sts.zscore(WPPRECTd[1,:])

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(WPPRECT,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of WPPRECTi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap_%s-%s_WPPRECTi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
