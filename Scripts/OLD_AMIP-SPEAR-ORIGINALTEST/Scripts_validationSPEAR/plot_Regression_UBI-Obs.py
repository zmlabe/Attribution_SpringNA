"""
Calculate regression maps of UBI onto T2M maps
...
UBId = np.genfromtxt(directorydata2 + 'UBI/UBI_%s_%s_%s-%s_detrended.txt' % ('Z500',monq,yearmin,yearmax),unpack=True)
years_UBI = UBId[0,:]
UBI = sts.zscore(UBId[1,:])

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(UBI,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of UBIi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap_%s-%s_UBIi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
