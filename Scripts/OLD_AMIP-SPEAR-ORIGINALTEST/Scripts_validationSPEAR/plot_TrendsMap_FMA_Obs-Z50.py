...
climo = np.nanmean(fma[yearclimoq,:,:],axis=0)
...
label = r'\textbf{Z50 Trend m/decade]}'
...
varc, lons_cyclicc = addcyclic(climo, lon1)
varc, lons_cyclicc = shiftgrid(180., varc, lons_cyclicc, start=False)
lon2dc, lat2dc = np.meshgrid(lons_cyclicc, lat1)
...
cs2 = m.contour(xc,yc,varc,np.arange(19700,20200,100),extend='both',colors='k',linewidths=3)
# cs2 = m.contour(xc,yc,varc,np.arange(19100,20900,100),extend='both',colors='k',linewidths=3)
...
plt.savefig(directoryfigure + 'TrendsMap_%s_%s-%s_Z50.png' % (sliceperiod,yearmin,yearmax),dpi=300)
