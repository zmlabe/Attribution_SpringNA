"""
Plot composites of different AO/PNA combinations during period of interest
...
for v in range(len(varplot)):
    ...
    posPNAposAOq = np.where((PNA > 0) & (AO > 0))[0]
    posPNAnegAOq = np.where((PNA > 0) & (AO < 0))[0]
    negPNAnegAOq = np.where((PNA < 0) & (AO < 0))[0]
    negPNAposAOq = np.where((PNA < 0) & (AO > 0))[0]
    # if len(posPNAposAO) + len(posPNAnegAO) + len(negPNAnegAO) + len(negPNAposAO) != years.shape[0]:
    #     print('SOMETHING IS WRONG WITH COMPOSITES!')
    #     sys.exit()
    
    ### Calculate mean composites
    posPNAposAO = np.nanmean(vardt[posPNAposAOq,:,:],axis=0)
    posPNAnegAO = np.nanmean(vardt[posPNAnegAOq,:,:],axis=0)
    negPNAnegAO = np.nanmean(vardt[negPNAnegAOq,:,:],axis=0)
    negPNAposAO = np.nanmean(vardt[negPNAposAOq,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posPNAposAOq),len(posPNAnegAOq),len(negPNAposAOq),len(negPNAnegAOq)]
    allcomposites = [posPNAposAO,posPNAnegAO,negPNAposAO,negPNAnegAO]
    comptitles = ['+PNA,+AO','+PNA,-AO','-PNA,+AO','-PNA,-AO']
    
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ### Plot composite maps of different AO/PNA combinations
    if variqM == 'T2M':
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_AO-PNA-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_AO-PNA-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
