"""
Compare trends in the data for the AMIPS from 1979 - 2019
...
### Read data
if calctrends == True:
    fma = np.load(directorydata + 'AMIP_SPEARval_AllData_%s_%s.npz' % (slicemonth,variq),
                  allow_pickle = True)
    lat = fma['lat']
    lon = fma['lon']
    amip = fma['amip']
    ...
    np.savez(directorydata + 'MapsOfTrends_SPEARval_AMIPs_%s_%s_%s.npz' % (slicemonth,
                                                               variq,
                                                               trendper),
             lat=lat,lon=lon,trends=treeall)
    np.savez(directorydata + 'MapsOfTrends_SPEARval_AMIPs_EnsembleMean_%s_%s_%s.npz' % (slicemonth,
                                                               variq,
                                                               trendper),
...
plt.savefig(directoryfigure + 'TrendsMap_%s_AMIPs_%s_%s_%s.png'% (slicemonth,variq,trendper,scenario[exxp]),dpi=300)
z
