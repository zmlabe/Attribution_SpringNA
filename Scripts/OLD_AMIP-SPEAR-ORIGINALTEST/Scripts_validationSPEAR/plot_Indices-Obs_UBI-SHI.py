"""
Plot monthly trends in the Arctic for the UBI and SHI
...
ubid = np.genfromtxt(directorydata + 'UBI/UBI_%s_%s_%s-%s_detrended.txt' % ('Z500',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_ubi = ubid[0,:]
ubi = sts.zscore(ubid[1,:])

shid = np.genfromtxt(directorydata + 'SHI/SHI_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_shi = shid[0,:]
shi = sts.zscore(shid[1,:])
    
### Calculate trends
slope_ubi,intercept_ubi,r_ubi,p_ubi,se_ubi = sts.linregress(years,ubi)
slope_shi,intercept_shi,r_shi,p_shi,se_shi = sts.linregress(years,shi)

### Calculate correlations
corr_ubi,pcorr_ubi = sts.pearsonr(obsz,ubi)
corr_shi,pcorr_shi = sts.pearsonr(obsz,shi)
...
rects = plt.bar(years,shi,align='center',zorder=2,color='crimson',label=r'SHI',clip_on=False)
plt.plot(years,ubi,linestyle='-',color='darkblue',linewidth=1,clip_on=False,label='UBI')
...
plt.savefig(directoryfigure + 'UBI-SHI_Indices-Obs_%s' % monq,dpi=300)
