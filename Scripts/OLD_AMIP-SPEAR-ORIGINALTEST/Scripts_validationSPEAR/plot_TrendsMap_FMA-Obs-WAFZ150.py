"""
Plot trends in FMA from 1979 to 2022 for WAFz150
...
### Read data
data = Dataset(directorydata + 'WAFZ150_1979-2021.nc')
lat1 = data.variables['latitude'][:]
lon1 = data.variables['longitude'][:]
snd = data.variables['WAFZ150'][:].reshape(years.shape[0],12,lat1.shape[0],lon1.shape[0])
data.close()
...
limit = np.arange(-0.002,0.0021,0.0001)
barlim = np.round(np.arange(-0.002,0.004,0.002),4)

fig = plt.figure(figsize=(6,6))
label = r'\textbf{WAFZ150 Trend ["m$^{2}$/s$^{2}$/decade]}'
...
cs2 = m.contour(x,y,cl,np.arange(-0.03,0.031,0.005),extend='both',
...
plt.savefig(directoryfigure + 'TrendsMap_%s_%s-%s_WAFZ150.png' % (sliceperiod,yearmin,yearmax),dpi=300)
