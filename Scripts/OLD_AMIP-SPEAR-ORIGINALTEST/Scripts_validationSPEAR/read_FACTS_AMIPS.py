...
def read_FACTS_Experi(scenario,model,vari,slicemonth,sliceshape,slicenan,level):
    ...
    if scenario == 'amip_1880s_rf':
        ...
        if model == 'CAM4':
            ...
            print(ValueError('WRONG MODEL FOR AMIP EXPERIMENT!!!'))
            sys.exit()
    elif scenario == 'spear_obs_rf':
        directory = '/work/Zachary.Labe/Data/SPEAR/SPEAR_c192_pres_OISST_HIST_AllForc_IC1901_R50/monthly/'
        if model == 'SPEAR':
            yrmin = 1901
            yrmax = 2020
            numOfEns = 30
    ...
    if level == 'surface':
        ...
        for i,ensmember in enumerate(ens):
            if scenario == 'spear_obs_rf':
                filename = directory + '%s/%s_%02d_%s-%s.nc' % (vari,vari,ensmember,yrmin,yrmax)        
            else:
    ...
    ###########################################################################
    ### Change units
    if any([vari=='T2M',vari=='SST',vari=='TMIN',vari=='TMAX']):
...
# latss,lonss,levss,varss,yearsss = read_FACTS_Experi('spear_obs_rf','SPEAR',vari,slicemonth,sliceshape,slicenan,level)
