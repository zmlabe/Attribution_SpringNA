...
directorydataAMIP = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/AMIPs/'
...
### Read data
latobs,lonobs,var = ERA.read_ERA5_monthly1x1(variq,directorydata,sliceperiod,
...
amip = np.load(directorydataAMIP + 'AMIP_SPEARval_MonthlyData_AllMonths_T2M.npz')
...
for i in range(len(monthq)*2):
    ...
    if i < 12:
        plt.title(r'\textbf{%s}' % monthq[i],fontsize=12,color='dimgrey')
    if i == 0:
...
plt.savefig(directoryfigure + 'TrendsMap_Monthly-Arctic-%s_%s_%s-%s_%s.png' % (plottype,sliceperiod,yearmin,yearmax,variq),dpi=300)
...
plt.savefig(directoryfigure + 'TrendsMap_Monthly-Arctic-%s_%s_%s-%s_%s-DIFFERENCE.png' % (plottype,sliceperiod,yearmin,yearmax,variq),dpi=300)
...
ccc=ww.cubehelix1_16.mpl_colormap(np.linspace(0.1,0.6,len(modelname)))
for i in range(len(monthq)):
    plt.scatter(i,ave_obs[i],s=50,c='darkblue',edgecolor='darkblue',zorder=5,clip_on=False)
...
plt.text(6.3,1.5,r'\textbf{ERA5}',fontsize=10,color='darkblue')

plt.ylabel(r'\textbf{T2M Trend - %sN - [$^{\circ}$C/decade] for %s--%s}' % (latpolar,yearmin,yearmax),color='k',fontsize=6)    
plt.xticks(np.arange(0,12,1),monthq,fontsize=7)
plt.yticks(np.arange(-5,5.1,0.25),map(str,np.round(np.arange(-5,5.1,0.25),2)),fontsize=7)
plt.xlim([-1,12])
plt.ylim([0,1.5])
plt.tight_layout()
plt.savefig(directoryfigure + 'meanTrends_AMIPS_Arctic-%s_%s_%s_%s.png' % (latpolar,variq,yearmin,yearmax),dpi=300)
