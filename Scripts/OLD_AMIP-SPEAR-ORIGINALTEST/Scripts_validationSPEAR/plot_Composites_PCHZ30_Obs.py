"""
Plot composites of different PCHZ30 combinations during period of interest
...
for v in range(len(varplot)):
    ...
    PCHZ30d = np.genfromtxt(directorydata2 + 'PCHZ30/PCHZ30_%s_%s_%s-%s_detrended.txt' % ('Z30',monq,
                                                                    yearmin,yearmax),
                                                                    unpack=True)
    years_PCHZ30 = PCHZ30d[0,:]
    PCHZ30 = sts.zscore(PCHZ30d[1,:])
    
    posPCHZ30q = np.where((PCHZ30 > 0.25))[0]
    negPCHZ30q = np.where((PCHZ30 < -0.25))[0]

    ### Calculate mean composites
    posPCHZ30 = np.nanmean(vardt[posPCHZ30q,:,:],axis=0)
    negPCHZ30 = np.nanmean(vardt[negPCHZ30q,:,:],axis=0)
    
    ### Assemble for plotting
    nsample = [len(posPCHZ30q),len(negPCHZ30q)]
    allcomposites = [posPCHZ30,negPCHZ30]
    comptitles = ['+PCHZ30','-PCHZ30']
    
    ###############################################################################
    ###############################################################################
    ###############################################################################
    ### Plot composite maps of different PCHZ30/PCHZ30 combinations
    if variqM == 'T2M':
    ...
    if detrend == True:  
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_PCHZ30-%s_%s-%s_detrend.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
    else:
        plt.savefig(directoryfigure + 'CompositeMaps_%s-%s_PCHZ30-%s_%s-%s.png' % (yearmin,yearmax,monq,variqM,sliceperiod),dpi=300)
