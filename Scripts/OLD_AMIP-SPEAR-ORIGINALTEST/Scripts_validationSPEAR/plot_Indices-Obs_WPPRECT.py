"""
Plot monthly trends in the Arctic for the WPPRECT
...
WPPRECTd = np.genfromtxt(directorydata + 'WPPRECT/WPPRECT_%s_%s_%s-%s_detrended.txt' % ('P',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_WPPRECT = WPPRECTd[0,:]
WPPRECT = sts.zscore(WPPRECTd[1,:])
    
### Calculate trends
slope_WPPRECT,intercept_WPPRECT,r_WPPRECT,p_WPPRECT,se_WPPRECT = sts.linregress(years,WPPRECT)

### Calculate correlations
corr_WPPRECT,pcorr_WPPRECT = sts.pearsonr(obsz,WPPRECT)
print(corr_WPPRECT,pcorr_WPPRECT)
...
rects = plt.bar(years,WPPRECT,align='center',zorder=2,color='crimson',label=r'WPPRECT',clip_on=False)
...
plt.savefig(directoryfigure + 'WPPRECT_Indices-Obs_%s' % monq,dpi=300)
