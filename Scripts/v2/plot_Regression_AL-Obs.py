"""
Calculate regression maps of AL onto T2M maps
...
ALd = np.genfromtxt(directorydata2 + 'AL/AL_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,1979,2020),unpack=True)
years_AL = ALd[0,:]
AL = sts.zscore(ALd[1,:])
AL = AL[yearq]

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(AL,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of ALi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap_%s-%s_ALi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
