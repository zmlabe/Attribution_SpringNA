"""
Calculate correlation maps for loops of indices onto T2MBox_NA
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/v2/obs/ObsCorrelationPatterns/T2M_BoxNA/' 
...
for mo in range(len(monthsall)):
    for moo in range(len(monthsall)):
        ...
        for iii in range(len(variregress)):
            ...
            m = Basemap(projection='ortho',lon_0=251,lat_0=55,resolution='l',area_thresh=10000)
            ...
            lonsslice = np.linspace(lo1,lo2,lo2-lo1+1)
            latsslice = np.ones(len(lonsslice))*la2
            m.plot(lonsslice, latsslice, color='w', linewidth=1.5, latlon=True,zorder=4)
            latsslice = np.ones(len(lonsslice))*la1
            m.plot(lonsslice, latsslice, color='w', linewidth=1.5, latlon=True,zorder=4)
            m.drawgreatcircle(lo1, la1, lo1, la2,linewidth=1.5,color='w',zorder=4)
            m.drawgreatcircle(lo2, la2, lo2, la1,linewidth=1.5,color='w',zorder=4)
            ...
            plt.savefig(directoryfigure + 'ObsCorrelationPatterns_T2M_BoxNApattern_%s-%s_%s-%s.png' % (variqM,sliceperiod,'T2M_BoxNA',monq),dpi=300)
            
            ### Save file
            np.savez(directorydataoutput + 'ObsCorrelationPatterns_T2M_BoxNApattern_%s-%s_%s-%s.npz' % (variqM,sliceperiod,'T2M_BoxNA',monq),
