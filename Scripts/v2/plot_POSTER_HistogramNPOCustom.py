...
loadens_amip,min_amip = np.load(directorydata2 + 'EnsembleMembers_SimilarTrends_AMIPSPEAR_T2M_%s_1979-2020.npy' % mon_index,
                        allow_pickle=True)
...
for iii in range(len(indicesdir)):
    ...
    n_corrall, bins_corrall, patches_corrall = plt.hist(corrall,bins=np.arange(-1,1.01,0.05),
                                            density=False,alpha=0.8,
                                            label=r'\textbf{NPO* Index Correlations for SPEAR_MED_AMIP (30)}',
                                            zorder=5)
    
    color = cmr.ghostlight(np.linspace(0.3,1,5-1))
    for i in range(len(patches_corrall)):
        patches_corrall[i].set_facecolor(color[0])
        patches_corrall[i].set_edgecolor('white')
        patches_corrall[i].set_linewidth(1)
    ...
    plt.axvline(x=corrobs,linewidth=5,color='k',linestyle='--',dashes=(1,0.3),zorder=12)
    
    plt.text(corrobs,8.1,r'\textbf{ERA5}',size=11,color='k',ha='center',va='center',
             rotation=0)
    plt.ylabel(r'\textbf{Number of Ensemble Members}',fontsize=10,color='k')
    plt.xlabel(r'\textbf{R} [February-April -- 1979-2020]',fontsize=10,color='k')
    plt.yticks(np.arange(0,31,1),map(str,np.round(np.arange(0,31,1),2)),size=6)
    plt.xticks(np.arange(-1,1.1,0.1),map(str,np.round(np.arange(-1,1.1,0.1),2)),size=6)
    plt.xlim([-1,0.1])   
    plt.ylim([0,8])
        
    plt.tight_layout()
    plt.savefig(directoryfigure + 'POSTER_Histogram_r_1979-2020_AMIPSPEAR_%s_%s.png' % (indicesdir[iii],mon_index),
                dpi=600)
