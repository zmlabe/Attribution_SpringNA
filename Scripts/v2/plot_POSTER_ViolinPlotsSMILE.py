...
for i in range(len(datum)):
    ...
    plt.plot(y,x,color='k',alpha=0.5,zorder=10,marker='.',linewidth=0,markersize=5,markeredgewidth=0,clip_on=False)
...
color = cmr.rainforest(np.linspace(0.00,0.9,len(datum)))
...
vp['bodies'][5].set_alpha(0.8)
vp['bodies'][6].set_alpha(0.8) 

plt.xticks(np.arange(-5,5.01,0.5),map(str,np.arange(-5,5.01,0.5)))
plt.xlim([-0.5,1.2])  
plt.xlabel(r'\textbf{T2M Trends [$^{\circ}$C/Decade] for April %s-%s}' % (yrmin,yrmax),color='k',size=7)

axb.xaxis.set_ticks_position('bottom')
axb.yaxis.set_ticks_position('left')

plt.text(decobs,7.8,r'\textbf{ERA5}',size=11,color='k',ha='center',va='center')

plt.tight_layout()
plt.savefig(directoryfigure + 'POSTER_TrendViolins_CESM_%s_%s-%s.png' % (sliceperiod,yrmin,yrmax),dpi=300)
