...
for iii in range(len(indicesdir)):
    ...
    for i in range(len(patches_corrall)):
        patches_corrall[i].set_facecolor('deepskyblue')
    ...
    plt.axvline(x=corrobs,linewidth=5,color='crimson',linestyle='--',dashes=(1,0.3),zorder=12)
    
    plt.text(corrobs,8.1,r'\textbf{ERA5}',size=11,color='crimson',ha='center',va='center',
             rotation=0)
    plt.ylabel(r'\textbf{Number of Ensemble Members}',fontsize=10,color='w')
    plt.xlabel(r'\textbf{R} [February-April -- 1979-2020]',fontsize=10,color='w')
