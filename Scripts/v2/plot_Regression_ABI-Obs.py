"""
Calculate regression maps of ABI onto T2M maps
...
ABId = np.genfromtxt(directorydata2 + 'ABI/ABI_%s_%s_%s-%s_detrended.txt' % ('Z500',monq,1979,2020),unpack=True)
years_ABI = ABId[0,:]
ABI = sts.zscore(ABId[1,:])
ABI = ABI[yearq]

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(ABI,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of ABIi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap_%s-%s_ABIi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
