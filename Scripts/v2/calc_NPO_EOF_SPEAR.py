"""
Calculate EOF for NPO using SPEAR
...
directoryoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/v2/SPEAR/NPO/'
...
for i,yr in enumerate(range(yearsrange.min(),yearsrange.max()-timelength+1,1)):
    ...
    ### Read data
    lat1,lon1,var = SP.read_SPEAR_MED('/work/Zachary.Labe/Data/SPEAR/SPEAR_MED/monthly/',
                                        variq,sliceperiod,4,'nan',30,'all')
...
### Save data
np.savez(directoryoutput + 'LatLon_NPO_coupledSPEAR_%s.npz' % sliceperiod,
         minall_lat=minall_lat,maxall_lat=maxall_lat,
         minall_lon=minall_lon,maxall_lon=maxall_lon,lat1=lat1,lon1=lon1,
         lat2s=lat2s,lon2s=lon2s)
np.savez(directoryoutput + 'EOFinfo_NPO_coupledSPEAR_%s.npz' % sliceperiod,
         eofall=eofall,pcall=pcall,varianceall=varianceall,
         timelength=timelength,years=years,lat1=lat1,lon1=lon1,
         lat2s=lat2s,lon2s=lon2s)

### Save lat/lon grid for plotting
np.savez(directoryoutput + 'LatLonGrid_NPO_coupledSPEAR_%s.npz' % sliceperiod,
         lat2s=lat2s,lon2s=lon2s,lat1=lat1,lon1=lon1)
