"""
Calculate regression maps of NPOcustomZ onto T2M maps
...
NPOcustomZd = np.genfromtxt(directorydata2 + 'NPOcustomZ/NPOcustomZ_%s_%s_%s-%s_detrended.txt' % ('Z500',monq,1979,2020),unpack=True)
years_NPOcustomZ = NPOcustomZd[0,:]
NPOcustomZ = sts.zscore(NPOcustomZd[1,:])
NPOcustomZ = NPOcustomZ[yearq]

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(NPOcustomZ,vardt,'obs')
...
label = r'\textbf{Regression [$^{\circ}$C/SD]}'
...
m = Basemap(projection='ortho',lon_0=255,lat_0=70,resolution='l',area_thresh=10000)
m.drawcoastlines(color='w',linewidth=1)
# m.drawstates(color='dimgrey',linewidth=0.3)
m.drawcountries(color='w',linewidth=0.6)
...
#                 color='w',fontsize=4,zorder=6)
# mer=m.drawmeridians(meridians,labels=[True,True,True,True],linewidth=1,
#                     fontsize=4,color='w',zorder=6)

circle = m.drawmapboundary(fill_color='k',color='dimgrey',
                  linewidth=0.7)
...
m.plot(lonsslice, latsslice, color='gold', linewidth=4, latlon=True,zorder=7)
latsslice = np.ones(len(lonsslice))*la1
m.plot(lonsslice, latsslice, color='gold', linewidth=4, latlon=True,zorder=7)
m.drawgreatcircle(lo1, la1, lo1, la2,linewidth=4,color='gold',zorder=7)
m.drawgreatcircle(lo2, la2, lo2, la1,linewidth=4,color='gold',zorder=7)
 
cbar_ax1 = fig.add_axes([0.05,0.06,0.2,0.025])                
cbar1 = fig.colorbar(cs1,cax=cbar_ax1,orientation='horizontal',
                    extend='both',extendfrac=0.07,drawedges=False)
cbar1.set_label(label,fontsize=8,color='w',labelpad=1.4)  
...
plt.savefig(directoryfigure + 'PRESENT_RegressionMap_%s-%s_NPOcustomZi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=600)
