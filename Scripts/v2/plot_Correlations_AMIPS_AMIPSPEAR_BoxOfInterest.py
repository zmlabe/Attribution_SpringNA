...
for i in range(len(indices)):
    for mm in range(len(models)):
        ...
        plt.title(r'\textbf{Correlation Map for %s(%s) on %s(%s) using %s}' % (variregress[i],mon_t2m,indices[0],mon_index,models[mm]),fontsize=8,color='k')
        ...
        plt.savefig(directoryfigure + 'CorrelationPatternsAMIP-T2M_BoxNA_pattern_%s-%s_%s-%s_%s.png' % (variregress[i],mon_t2m,indices[0],mon_index,models[mm]),dpi=300)
        
        ### Save file
        np.savez(directorydataoutput + 'CorrelationPatternsAMIP-T2M_BoxNA_pattern_%s-%s_%s-%s_%s.npz' % (variregress[i],mon_t2m,indices[0],mon_index,models[mm]),
