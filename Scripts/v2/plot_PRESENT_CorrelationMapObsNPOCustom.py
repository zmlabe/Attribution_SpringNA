...
m.plot(lonsslice, latsslice, color=ccc, linewidth=4, latlon=True,zorder=4)
latsslice = np.ones(len(lonsslice))*la1
m.plot(lonsslice, latsslice, color=ccc, linewidth=4, latlon=True,zorder=4)
m.drawgreatcircle(lo1, la1, lo1, la2,linewidth=4,color=ccc,zorder=4)
m.drawgreatcircle(lo2, la2, lo2, la1,linewidth=4,color=ccc,zorder=4)
...
plt.savefig(directoryfigure + 'PRESENT_ObsCorrelationPatterns_T2M_BoxNApattern_%s-%s_%s-%s.png' % (variqM,sliceperiod,'T2M_BoxNA',sliceperiod),dpi=600)
