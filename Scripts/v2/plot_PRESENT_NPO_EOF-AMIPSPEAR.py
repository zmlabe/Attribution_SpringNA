"""
Plot EOF for NPO using AMIP-SPEAR data
...
directorydata = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/v2/AMIPs/NPO/' 
...
### Read in data
eof = np.load(directorydata + 'EOFinfo_NPO_AMIPSPEAR_%s.npz' % sliceperiod)
eofalln = eof['eofall']
variancealln = eof['varianceall']
timelength = eof['timelength']
yearsall = eof['years']

grid = np.load(directorydata + 'LatLon_NPO_AMIPSPEAR_%s.npz' % sliceperiod)
latmin = grid['minall_lat']
latmax = grid['maxall_lat']
lonmin = grid['minall_lon']
lonmax = grid['maxall_lon']

matrix = np.load(directorydata + 'LatLongird_NPO_AMIPSPEAR_%s.npz' % sliceperiod)
lat2s = matrix['lat2s']
lon2s = matrix['lon2s']
...
label = r'\textbf{EOF2 - NPO -- AMIP-SPEAR}'
...
for i in range(len(eofpatterns_all)):
    ...
    plt.text(104,26,r'\textbf{%s}' % yearslist[i],rotation=90,color='w',fontsize=14)   
...
plt.savefig(directoryfigure + 'PRESENT_EOF2_NPO_%s_AllTimePeriods_AMIPSPEAR.png' % sliceperiod,dpi=300)
...
plt.plot(years[:len(maxlonm)],maxlonm,linestyle='-',color='crimson',linewidth=2,clip_on=False,label='Northern Node')
plt.plot(years[:len(minlonm)],minlonm,linestyle='--',dashes=(1,0.3),color='deepskyblue',linewidth=2,clip_on=False,label='Southern Node')
...
plt.ylabel(r'\textbf{Longitude [$^{\circ}$]}',fontsize=8,color='darkgrey')
plt.title(r'\textbf{Shift in the NPO centers of action - %s -- AMIP-SPEAR}' % sliceperiod,fontsize=12,color='darkgrey')
plt.savefig(directoryfigure + 'PRESENT_EOF2_NPOshift_AMIP_%s.png' % sliceperiod,dpi=300)
