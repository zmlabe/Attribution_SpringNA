"""
Plot timeseries of NPOcustomcustom
...
NPOcustomd = np.genfromtxt(directorydata + 'NPOcustom/NPOcustom_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_NPOcustom = NPOcustomd[0,:]
NPOcustom = sts.zscore(NPOcustomd[1,:])
    
### Calculate trends
slope_NPOcustom,intercept_NPOcustom,r_NPOcustom,p_NPOcustom,se_NPOcustom = sts.linregress(years,NPOcustom)
print(slope_NPOcustom,p_NPOcustom)

### Calculate correlations
corr_NPOcustom,pcorr_NPOcustom = sts.pearsonr(obsz,NPOcustom)
print('\n',corr_NPOcustom,pcorr_NPOcustom)

### Smooth NPOcustom index
smoothed = sm.nonparametric.lowess(NPOcustom,np.arange(years.shape[0]),
...
rects = plt.bar(years,NPOcustom,align='center',zorder=2,color='crimson',label=r'NPOcustom',clip_on=False)
...
plt.savefig(directoryfigure + 'NPOcustom_Indices-Obs_%s_detrended' % monq,dpi=300)
