...
ax = plt.subplot(111)
ax.xaxis.grid(zorder=1,color='dimgrey',alpha=1,clip_on=False)
...
plt.axhline(decobs,color='darkgrey',linestyle='--',dashes=(1,0.3),linewidth=7)

def autolabel(rects):
    for rect in rects:
        height = rect.get_height()
        if rect.get_height() > 0:
            ax.text(rect.get_x() + rect.get_width()/2., rect.get_height()-0.007,
                    r'\textbf{+%2.2f}' % height,ha='center',va='top',
                    zorder=11,color='dimgrey',fontsize=7)
        elif rect.get_height() <= 0:
            ax.text(rect.get_x() + rect.get_width()/2., rect.get_height()+0.014,
                    r'\textbf{%2.2f}' % height,ha='center',va='top',
                    zorder=11,color='dimgrey',fontsize=7)
autolabel(rects1)
autolabel(rects2)
...
ylabels = map(str,np.round(np.arange(-2,2.1,0.1),2))
plt.yticks(np.arange(-2,2.1,0.1),ylabels,fontsize=6,color='k')
plt.ylim([-0.2,0.4])
plt.xlim([0,5])

plt.text(1.4,-0.175,r'\textbf{ERA5 Observed Trend}',color='darkgrey',fontsize=11)

plt.title(r'\textbf{FORCING CONTRIBUTIONS}',fontsize=23,color='w',alpha=1) 
plt.ylabel(r'\textbf{T2M Trends [$^{\circ}$C/Decade] for February-April %s-%s' % (yrmin,yrmax),color='k',size=10)

plt.tight_layout()
plt.subplots_adjust(top=0.9)
plt.savefig(directoryfigure + 'POSTER_ForcingContributions_%s_%s-%s.png' % (sliceperiod,yrmin,yrmax),dpi=300)
