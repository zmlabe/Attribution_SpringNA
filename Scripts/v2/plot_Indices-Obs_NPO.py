...
NPOd = np.genfromtxt(directorydata + 'NPO/NPO_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_NPO = NPOd[0,:]
NPO = sts.zscore(NPOd[1,:])
    
### Calculate trends
slope_NPO,intercept_NPO,r_NPO,p_NPO,se_NPO = sts.linregress(years,NPO)

### Calculate correlations
corr_NPO,pcorr_NPO = sts.pearsonr(obsz,NPO)
print(corr_NPO,pcorr_NPO)

### Smooth NPO index
smoothed = sm.nonparametric.lowess(NPO,np.arange(years.shape[0]),
...
rects = plt.bar(years,NPO,align='center',zorder=2,color='crimson',label=r'NPO',clip_on=False)
...
plt.savefig(directoryfigure + 'NPO_Indices-Obs_%s_detrended' % monq,dpi=300)
