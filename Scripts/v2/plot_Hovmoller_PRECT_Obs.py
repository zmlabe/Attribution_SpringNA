"""
Plot trends in spring from 1979 for PRECT
...
letters = ["a","b","c","d","e","f","g","h","i","j","k","l","m","n"]
limit = np.arange(-2.01,2.011,0.001)
barlim = np.round(np.arange(-2,3,1),2)
cmap = cmocean.cm.tarn
label = r'\textbf{PRECT Anomalies [mm/day] -- ERA5}'
...
plt.savefig(directoryfigure + 'P_%s_AllTimePeriods_Obs.png' % sliceperiod,dpi=300)
...
for i in range(len(sst_all)):
    ...
    plt.text(70,-7,r'\textbf{%s}' % yearslist[i],rotation=90,color='k',fontsize=14)   
