"""
Plot regression patterns for FLOR in the extratropics indices
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/v2/FLOR/RegressionPatternsFLOR/' 
...
directorydataoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/v2/FLOR/'
...
for i in range(len(indices)):
    for mm in range(len(models)):
        ### Read in FLOR indices
        amip = np.load(directorydata + 'ClimateIndices_coupledFLOR_%s_1979-2020.npz' % mon_index)
        amipi = amip['FLOR'][i,:,:]

        ### Read in AMIP data
        lat1q,lon1q,FLOR = FL.read_FLOR('/work/Zachary.Labe/Data/',
                                        variq,mon_t2m,4,'nan',30,'satellite')
        
        ### Calculate regional mean
        anom = calc_anomalies(years,FLOR)
        ...
        plt.savefig(directoryfigure + 'RegressionFLOR_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),dpi=300)
        
        ### Save file
        np.savez(directorydataoutput + 'RegressionFLOR_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),
