"""
Calculate regression maps of NINO34 onto T2M maps
...
NINO34d = np.genfromtxt(directorydata2 + 'NINO34/NINO34_%s_%s_%s-%s_detrended.txt' % ('SST',monq,yearmin,yearmax),unpack=True)
years_NINO34 = NINO34d[0,:]
NINO34 = sts.zscore(NINO34d[1,:])

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(NINO34,vardt,'obs')
...
label = r'\textbf{Regression Coefficients [$^{\circ}$C/SD]}'
...
circle = m.drawmapboundary(fill_color='k',color='dimgray',
                  linewidth=0.7)
circle.set_clip_on(False)

varnew = var*pvar
varnew[np.where(varnew == 0.)] =np.nan
cs1 = m.contourf(x,y,varnew,limit,extend='both')
...
plt.savefig(directoryfigure + 'PRESENT_RegressionMap_%s-%s_NINO34i-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
