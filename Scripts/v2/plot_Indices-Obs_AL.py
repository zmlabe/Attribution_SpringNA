...
obs = np.genfromtxt(directorydata + 'T2M_BoxNA/T2M_BoxNA_%s_%s_%s-%s.txt' % ('T2M',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
obsz = sts.zscore(obs[1,:])

### Read in teleconnections
ALd = np.genfromtxt(directorydata + 'AL/AL_%s_%s_%s-%s.txt' % ('SLP',monq,
                                                                yearmin,yearmax),
                                                                unpack=True)
years_AL = ALd[0,:]
AL = sts.zscore(ALd[1,:])
...
### Calculate trends
slope_AL,intercept_AL,r_AL,p_AL,se_AL = sts.linregress(years,AL)

### Calculate correlations
corr_AL,pcorr_AL = sts.pearsonr(obsz,AL)
print(corr_AL,pcorr_AL)
...
rects = plt.bar(years,AL,align='center',zorder=2,color='crimson',label=r'AL',clip_on=False)
...
plt.savefig(directoryfigure + 'AL_Indices-Obs_%s' % monq,dpi=300)
