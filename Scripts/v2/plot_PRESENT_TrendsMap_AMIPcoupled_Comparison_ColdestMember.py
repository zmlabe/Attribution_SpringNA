...
loadens_amip,min_amip = np.load(directorydata + 'EnsembleMembers_SimilarTrends_AMIPSPEAR_T2M_%s_1979-2020.npy' % slicemonth,
                        allow_pickle=True)
loadens_coupled,min_coupled = np.load(directorydata2 + 'EnsembleMembers_SimilarTrends_CoupledSPEAR_T2M_%s_1979-2020.npy' % slicemonth,
                        allow_pickle=True)
...
for i in range(len(plotdata)):
    ...
    m.drawcoastlines(color='dimgrey',linewidth=0.75)
    m.drawcountries(color='dimgrey',linewidth=0.4)
    ...
    plt.title(r'\textbf{%s}' % experi1[i],fontsize=8,color='w')
...
plt.savefig(directoryfigure + 'PRESENT_TrendsMap_%s_AMIPsCoupled_%s_%s_ColdestMember.png' % (slicemonth,variq,trendper),dpi=300)
