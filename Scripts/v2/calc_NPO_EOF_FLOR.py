"""
Calculate EOF for NPO using FLOR
...
directoryoutput = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/v2/FLOR/NPO/'
...
for i,yr in enumerate(range(yearsrange.min(),yearsrange.max()-timelength+1,1)):
    ...
    ### Read data
    lat1,lon1,var = FL.read_FLOR('/work/Zachary.Labe/Data/',
...
### Save data
np.savez(directoryoutput + 'LatLon_NPO_FLOR_%s.npz' % sliceperiod,
...
np.savez(directoryoutput + 'EOFinfo_NPO_FLOR_%s.npz' % sliceperiod,
...
np.savez(directoryoutput + 'LatLonGrid_NPO_FLOR_%s.npz' % sliceperiod,
