...
plt.savefig(directoryfigure + 'PRESENT_EOF2_NPO_%s_AllTimePeriods.png' % sliceperiod,dpi=600)
...
plt.plot(years[:len(minlon)],minlon,linestyle='-',color='crimson',linewidth=2,clip_on=False,label='Nothern Node')
plt.plot(years[:len(maxlon)],maxlon,linestyle='--',dashes=(1,0.3),color='deepskyblue',linewidth=2,clip_on=False,label='Southern Node')
...
plt.ylabel(r'\textbf{Longitude [$^{\circ}$]}',fontsize=8,color='w')
plt.title(r'\textbf{Shift in the NPO centers of action - %s}' % sliceperiod,fontsize=12,color='w')
plt.savefig(directoryfigure + 'PRESENT_EOF2_NPOshift_obs_%s.png' % sliceperiod,dpi=600)
