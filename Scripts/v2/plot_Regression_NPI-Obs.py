"""
Calculate regression maps of NPI onto T2M maps
...
NPId = np.genfromtxt(directorydata2 + 'NPI/NPI_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,1979,2020),unpack=True)
years_NPI = NPId[0,:]
NPI = sts.zscore(NPId[1,:])
NPI = NPI[yearq]

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(NPI,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of NPIi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
plt.savefig(directoryfigure + 'RegressionMap_%s-%s_NPIi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
