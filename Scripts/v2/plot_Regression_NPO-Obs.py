"""
Calculate regression maps of NPO onto T2M maps
...
NPOd = np.genfromtxt(directorydata2 + 'NPO/NPO_%s_%s_%s-%s_detrended.txt' % ('SLP',monq,1979,2020),unpack=True)
years_NPO = NPOd[0,:]
NPO = sts.zscore(NPOd[1,:])
NPO = NPO[yearq]

### Calculate regression
slope,intercept,rvalue,pvalue,stderr = regressData(NPO,vardt,'obs')
...
plt.title(r'\textbf{%s-%s of NPOi(%s)-T2M(%s)}' % (yearmin,yearmax,monq,sliceperiod),fontsize=15,color='k')
...
# plt.savefig(directoryfigure + 'RegressionMap_%s-%s_NPOi-%s_T2M-%s.png' % (yearmin,yearmax,monq,sliceperiod),dpi=300)
