...
for r in range(len(plottrend20)):
    if r < 6:
        var = plottrend20[r]
        # pvar = plotpval20[r]
    else:
    ...
    if r < 6:
        plt.title(r'\textbf{%s}' % datalabels[r],color='w',fontsize=15)
...
###############################################################################
cbar_ax1 = fig.add_axes([0.352,0.125,0.3,0.03])                
cbar1 = fig.colorbar(cs,cax=cbar_ax1,orientation='horizontal',
                    extend='both',extendfrac=0.07,drawedges=False)
cbar1.set_label(label,fontsize=10,color='darkgrey',labelpad=0.7)  
...
plt.tight_layout()
plt.subplots_adjust(hspace=0.01,wspace=0.01,bottom=0.17,top=0.90)
plt.savefig(directoryfigure + 'PRESENT_differencesObservations_MapsOfTrends_%s.png' % sliceperiod,dpi=600)
