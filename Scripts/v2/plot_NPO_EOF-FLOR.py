"""
Plot EOF for NPO using FLOR data
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/v2/FLOR/NPO/' 
directorydata = '/home/Zachary.Labe/Research/Attribution_SpringNA/Data/v2/FLOR/NPO/' 
...
### Read in data
eof = np.load(directorydata + 'EOFinfo_NPO_FLOR_%s.npz' % sliceperiod)
eofalln = eof['eofall']
variancealln = eof['varianceall']
timelength = eof['timelength']
yearsall = eof['years']

grid = np.load(directorydata + 'LatLon_NPO_FLOR_%s.npz' % sliceperiod)
latmin = grid['minall_lat']
latmax = grid['maxall_lat']
lonmin = grid['minall_lon']
lonmax = grid['maxall_lon']

matrix = np.load(directorydata + 'LatLonGrid_NPO_FLOR_%s.npz' % sliceperiod)
lat2s = matrix['lat2s']
lon2s = matrix['lon2s']
...
plt.savefig(directoryfigure + 'EOF2_NPO_%s_AllTimePeriods_FLOR.png' % sliceperiod,dpi=300)
...
plt.title(r'\textbf{Shift in the NPO centers of action - %s -- FLOR}' % sliceperiod,fontsize=12,color='k')
plt.savefig(directoryfigure + 'EOF2_NPOshift_FLOR_%s.png' % sliceperiod,dpi=300)
