...
plt.axvline(decobs,color='k',linestyle='--',dashes=(1,0.3),linewidth=3)
...
plt.xticks(np.arange(-5,5.01,0.5),map(str,np.arange(-5,5.01,0.5)),rotation=270)
plt.xlim([-0.5,1.2])  
plt.xlabel(r'\textbf{T2M Trends [$^{\circ}$C/Decade] for April %s-%s}' % (yrmin,yrmax),
           color='k',size=13,rotation=180)

axb.xaxis.set_ticks_position('bottom')
axb.yaxis.set_ticks_position('left')

plt.text(decobs,4.87,r'\textbf{ERA5}',size=11,color='k',ha='center',va='center',
         rotation=270)

plt.yticks(rotation=270,va='center',fontsize=7.2) 

plt.tight_layout()
plt.savefig(directoryfigure + 'POSTER_TrendViolins_GFDL_%s_%s-%s.png' % (sliceperiod,yrmin,yrmax),dpi=300)
