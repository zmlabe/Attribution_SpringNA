...
cbar.ax.tick_params(axis='x',size=.001)
cbar.outline.set_edgecolor('dimgrey')
cbar.set_label(r'\textbf{Correlation Matrix - %s for %s-T2M}' % (monq,monq),
               color='dimgrey',labelpad=3,fontsize=12)

plt.tight_layout()
plt.savefig(directoryfigure + 'CorrelationMatrix_ClimateIndices-Obs_%s_lead_%s-%s' % (monq,yearmn,yearmx),dpi=300)
