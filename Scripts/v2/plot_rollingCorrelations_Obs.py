...
### Read data
years_era,tas_era = np.genfromtxt(directorydata + 'T2M_BoxNA_T2M_%s_1979-2020_detrended.txt' % months,
                     unpack=True)
years_era,al_era = np.genfromtxt(directorydata2 + '%s_%s_%s_1979-2020_detrended.txt' % (indexName,indexVar,months),
                     unpack=True)
...
plt.ylabel(r'\textbf{Correlation Coefficient}',fontsize=11,
                      color='dimgrey')
plt.xlabel(r'\textbf{First Year In Window [%syr]}' % window,fontsize=11,color='dimgrey')
plt.title(r'\textbf{Rolling Correlation between %s and T2M for %s}' % (indexName,months),
                    color='k',fontsize=11)
plt.tight_layout()        

### Save figure
plt.savefig(directoryfigure+'%s_rollingCorrelations%s_%s-T2M.png' % (indexName,window,months),dpi=300)
