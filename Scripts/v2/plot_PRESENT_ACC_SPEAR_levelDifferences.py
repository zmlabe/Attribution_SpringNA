...
for yr in range(len(slicemonthall)):
    ...
    if monthhind == 'Jan':
        if slicemonth == 'FMA':
            ...
            obs_time = np.nanmean(obs[:,0:1,:,:],axis=1)
    elif monthhind == 'Feb':
        if slicemonth == 'FMA':
    ...
    def calc_Corrs(spear,lat,lon,obs):
        ...
        if monthhind == 'Jan':
            if slicemonth == 'FMA':
                ...
                spear_time = np.nanmean(spear[:,:,2:3,:,:],axis=1)
            elif slicemonth == 'Feb':
                spear_time = np.nanmean(spear[:,:,1:2,:,:],axis=1)
            elif slicemonth == 'Jan':
                spear_time = np.nanmean(spear[:,:,0:1,:,:],axis=1)
        elif monthhind == 'Feb':
            if slicemonth == 'FMA':
            ...
            if slicemonth == 'FMA':
                ...
                spear_time = np.nanmean(spear[:,:,0:1,:,:],axis=2)
        else:
            print(ValueError('FULL TIME PERIOD NOT AVAILABLE!'))
            sys.exit()
    ...
    for pl in range(len(varplot)):
        ...
        if pl < 2:
            ...
            pvar[np.where(pvar >= 0.05)] = 0.
            pvar[np.where(pvar < 0.05)] = 1.
            pvar,lons_cyclic = addcyclic(pvar, lon)
            varnew = var*pvar
            varnew[np.where(varnew == 0.)] =np.nan
            cs1 = m.contourf(x,y,var*pvar,limit,extend='both')
        else:
        ...
        plt.title(r'%s' % experi1[pl],fontsize=12,color='w')
    ...
    cbar1.set_label(label,fontsize=9,color='w',labelpad=1.4)  
    ...
    plt.tight_layout()
    if detrend == False:
        plt.savefig(directoryfigure + 'PRESENT_ACC_%s_levelDifferences_init-%s_%s_%s-%s.png'% (slicemonth,monthhind,variq,yearmin,yearmax),dpi=300)
    else: 
        plt.savefig(directoryfigure + 'PRESENT_ACC_%s_levelDifferences_init-%s_%s_%s-%s_detrend.png'% (slicemonth,monthhind,variq,yearmin,yearmax),dpi=300)
