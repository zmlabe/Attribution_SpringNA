"""
Plot histogram of indices for correlations between index and obs in SPEAR
in the tropics
...
directoryfigure = '/home/Zachary.Labe/Research/Attribution_SpringNA/Figures/v2/SPEAR/PDFs/' 
...
loadens_amip,min_amip = np.load(directorydata2 + 'EnsembleMembers_SimilarTrends_CoupledSPEAR_T2M_%s_1979-2020.npy' % mon_index,
                        allow_pickle=True)

for iii in range(len(indicesdir)):
    
    ### Read in AMIP data
    amip = np.load(directorydata2 + 'ClimateIndices-Tropical_coupledSPEAR_%s_1979-2020.npz' % mon_index)
    ...
    amiptas = np.load(directorydata2 + 'ClimateIndices_coupledSPEAR-T2M_BoxNA_%s_1979-2020.npz' % mon_t2m)
    ...
    plt.xlabel(r'\textbf{R} [SPEAR - %s -- 1979-2020]' % mon_index,fontsize=10,color='k')
    ...
    plt.savefig(directoryfigure + 'Histogram_r_1979-2020_SPEAR_%s_%s.png' % (indicesdir[iii],mon_index),
                dpi=300)
