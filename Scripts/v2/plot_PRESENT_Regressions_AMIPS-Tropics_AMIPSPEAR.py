...
for i in range(len(indices)):
    for mm in range(len(models)):
        ...
        # amip = np.load(directoryoutput + 'ClimateIndices-Tropical_AMIPSPEAR_%s_1979-2020.npz' % mon_index)
        ...
        # pvalueM[np.where(np.isnan(pvalueM))] = 0.
        ###############################################################################
        ###############################################################################
        ###############################################################################
        ...
        cs1 = m.contourf(lon2,lat2,varnew,limit,extend='both',latlon=True)
        ...
        plt.savefig(directoryfigure + 'PRESENT_RegressionAMIP_pattern_%s-%s_%s-%s_%s.png' % (variq,mon_t2m,indices[i],mon_index,models[mm]),dpi=600)
